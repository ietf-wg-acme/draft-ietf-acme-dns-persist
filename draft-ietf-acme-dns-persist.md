...
3.  The issue-value MUST contain an `accounturi` parameter whose value is a URI that identifies the ACME account requesting validation, either directly or through a subscriber-account association.
    ...
    *   **(3d) ACME Public key**: CAs MAY accept a URI that starts with pubkey: and contains a Base64_URL encoded sha256 hash of the account JWK public key as described in ({{!RFC7638}}, Section 3). If a CA supports this mode, it MUST enforce the following constraints:
        ...
        2.  The CA MUST revoke any authorizations that have been validated using this mode if a successful keyChange as described in ({{!RFC8555}}, Section 7.3.5) is made, by transistoning it to "revoked". Note that a CA MUST NOT revoke an authorization that has another state than "valid" as a result of a keyChange, as this would violate ({{!RFC8555}}, Section 7.3.5).
...
- **Mode 3d (ACME Public key):** The client can verify the provisioned
  ...
  connectiviity, for example before DHCP has been completed or beofre
